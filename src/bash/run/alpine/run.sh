#!/usr/bin/env bash
# purpose:
# the runnable script on alpine 20.04 - runs generically the runnable functions
#
   ...
   img=alpine-3.12.0
	while [[ $# -gt 0 ]]; do
		case "$1" in
			...
			-l|--link-api-key) shift && LINK_API_KEY="${1:-} " && shift ;;
			-p|--pets-api-key) shift && PETS_API_KEY="${1:-} " && shift ;;
			-u|--uri-for-api) shift && API_URI="${1:-} " && shift ;;
...
do_run_actions(){
   actions=$1
      ...
      while read -d ' ' arg_action ; do
         ...
         done < <(find "src/bash/run/alpine/alpine-3.12.0" -type f -name '*.func.sh'|sort)
...
do_deploy_min_req_bins(){
	which perl 2> /dev/null || {
      sudo apk update && sudo apk upgrade && sudo apk add perl
   }
	which jq 2> /dev/null || {
      sudo apk update && sudo apk upgrade && sudo apk add jq
   }
	which curl 2> /dev/null || {
      sudo apk update && sudo apk upgrade && sudo apk add curl
   }
}
...
do_load_functions(){
    ...
    while read -r f; do source $f; done < <(ls -1 $PRODUCT_DIR/src/bash/run/alpine/$img/*.func.sh)
 }
