...
do_run: demand_var-ENV
	docker exec -e ENV=$$ENV -it proj-devops-con /opt/min-aws-cli/run -a do_run_hello_world
...
spawn_tgt_project: demand_var-TGT_PROJ demand_var-ENV zip_me
	...
	ENV=dev to_srch=min-aws-cli to_repl=$(shell echo $$TGT_PROJ) dir_to_morph=$(shell echo $(dir $(abspath $(dir $$PWD)))$$TGT_PROJ) ./run -a do_morph_dir
